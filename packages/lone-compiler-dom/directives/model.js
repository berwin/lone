  ...
  addHandler(el, 'change',
    ...
    'if(Array.isArray($$a)){' +
      `var $$v=${number ? '_n(' + valueBinding + ')' : valueBinding},` +
          '$$i=_i($$a,$$v);' +
      `if($$el.checked){$$i<0&&(slave.send('page:data', getLogicChannel(), {id: id, data:{${value}: $$a.concat([$$v])}}))}` +
      `else{$$i>-1&&(slave.send('page:data', getLogicChannel(), {id: id, data:{${value}: $$a.slice(0,$$i).concat($$a.slice($$i+1))}}))}` +
